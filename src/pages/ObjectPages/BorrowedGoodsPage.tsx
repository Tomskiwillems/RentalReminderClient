...
const BorrowedGoodsPage: React.FC = () => {
    ...
    useEffect(() => {
        fetch("/api/borrowed-goods", { credentials: "include" })
            .then(res => res.json())
            .then(json => {
    ...
    const handleNew = () => navigate("/borrowed-goods/add");

    const handleEdit = () => {
        if (selectedRow) {
            navigate(`/borrowed-goods/edit/${selectedRow.id}`);
        }
    };
    ...
    const confirmDelete = async () => {
        ...
        await fetch(`/api/borrowed-goods/delete/${selectedRow.id}`, {
            method: "DELETE",
            credentials: "include"
        }).catch(() => {});
    ...
    return (
        <ObjectPageLayout
            ...
            confirmMessage="Are you sure you want to delete this borrowed good?"
